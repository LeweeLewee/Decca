...
- Original **Stereo/Mono** contact assigned to GPIO25/D25: Stereo open requests
  lights on; Mono closed requests lights off. Firmware logic is correct, but the
  retained switch requires repair under HW-SW-01.
...
| FR-BTN-05 | The system shall debounce the Stereo/Mono contact on GPIO25 and expose its stable lighting request continuously: Stereo open/high = on; Mono closed/low = off. | 1 |
...
> phone-controlled network playback path (ADR-0016).
...
| ID     | Interface                                                                       |
|--------|---------------------------------------------------------------------------------|
| IF-01  | **Front panel:** four 10 kΩ pots on ADC1; retained on/off switch on GPIO14/D14; sole debounced VHF source contact on GPIO26/D26; pulled-up Stereo/Mono lighting-request contact on GPIO25/D25. VHF closed = Digital Streamer; VHF open = Vinyl. Stereo open/high = lights requested on; Mono closed/low = off. The retained Stereo/Mono switch fault is tracked as HW-SW-01. |
...
| IF-03  | **Lighting:** GPIO18/D18 PWM-driven warm dial illumination via logic-level N-channel MOSFET (dial only in Phase 1). |
