...
| Heat-set threaded insert, M3 | 2 required for the housing lid | **ACQUIRED — DIMENSIONS NOT YET MEASURED** | Hanglife M3 threaded heat-set inserts (owner, 2026-09-06). Two are pressed vertically into the +X lid-screw bosses of `Housing_Base`; the two the adjustable clamp needed were deleted with it under specification v1.7 §4.6. The housing currently models a **Ø4.00 × 5.00 mm deep bore**, which is the common recommendation for a 4.6 mm OD, 5.0 mm long M3 brass insert. **Measure the acquired insert's length and outer diameter** and re-derive the bore before any housing part is printed. | Existing stock |
| M3 machine screw, lid | 2 required for the housing lid | **NOT YET SELECTED** | Pass through Ø3.40 clearance holes in `Housing_Lid` at (x 34.50, y ±26.00) into the heat-set inserts above. Length follows from the lid thickness plus the insert depth once that insert is measured. | TBD |
| M3 countersunk screw, cabinet fixing | 2 required for the housing | **NOT YET ACQUIRED** | Fixes `Housing_Base` to the cabinet through two recessed, capped fixings on the centreline at x ±24.00. The countersink is cut for a **declared maximum head envelope of 6.20 mm** plus 0.25 mm radial clearance, from an assumed ISO 10642 head — **not** from a measured screw. Measure the acquired screw's head diameter and angle before printing; no compatibility is claimed until then. | TBD |
| ESP32 screw-terminal adapter | 1 required; 2 supplied | **ACQUIRED — MODEL AND OVERALL HEIGHT CONFIRMED / OTHER FEATURE MEASUREMENTS OPEN** | DORHEA “ESP32 Expansion Board 30Pin GPIO Breakout Board 1 into 2 Module Terminal Adapter”, compatible with the selected 30-pin ESP32 DevKit. **Measured by the owner on 2026-09-06:** PCB outline **66 mm across the board × 63 mm along the terminal rows** (confirming the supplier-stated plan envelope; the 66 mm is measured from one connector side to the other), mounting-hole centre pitch **60 mm across × 57 mm along** (re-measured the same day, superseding an earlier 58 × 56 reading; every hole then sits 3.0 mm from its nearest board edge), corner mounting holes taking **M3**, terminal blocks **55 mm outer face to outer face** (so each block face sits 5.5 mm inboard of its long PCB edge), row length **53 mm** and block height **9 mm**. The supplier image shows 15 top-operated screw terminals per long side with conductor ports facing horizontally outwards. The black vertical sockets carry the ESP32 module and are not external cable connections. **The complete adapter plus fitted ESP32 measures 20 mm high overall** (owner measurement, 2026-09-05). A further supplier dimension of 25 mm is shown without sufficiently clear datums and must not be used for CAD. Terminal-port position, board thickness and underside features remain to be measured before releasing a housing — see `mechanical/Drawings/Decca_ESP32_Controller_Housing_Measurement_Request.md` for the labelled list and datums. | Amazon / DORHEA |
