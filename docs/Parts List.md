...
| 10 kΩ D18 control pull-down resistor | 1 | **RECOMMENDED — DEFERRED BY OWNER** | If later fitted, connect from the DFR0457 control/PWM input (the same node as ESP32 GPIO18/D18) to common GND with all power off. It would provide defined reset-time safe-off before firmware controls D18. | Existing stock / _TBD_ |
