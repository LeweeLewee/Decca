...
|------|-----|-------------------------------|-------|--------|
| WiiM Pro | 1 | **LOCKED — ACQUIRED / NETWORK VERIFIED** | Delivered and configured as Decca on 2026-09-11. Fixed local address is held only in installation configuration. Live HTTPS `getStatusEx` and `getPlayerStatus` passed on firmware `Linkplay.4.8.827634`; idle `getMetaInfo` returned an empty body and active TIDAL metadata remains a bench gate. Continuously powered with automatic standby. | _Acquired_ |
