...
4. Confirm the OLED reveals the `DECCA` wordmark from left to right over roughly
   1 s, then holds `MUSIC CENTRE v0.27.4` for roughly 2.2 s in the lower
...
4. Observe the dial lighting. It should switch immediately from off to the test
   duty and immediately back to off, without flashing at full brightness or
   waiting before the final change.
5. Confirm the output contains `LIGHTING_SNAPSHOT duty=217`, the lamps hold at
   the approved 85% duty for five seconds, and all six tests pass.

Pass criteria:

- the light starts and finishes fully off;
- brightness changes immediately in both directions;
- no full-brightness flash occurs at reset or test start;
- the MOSFET and wiring remain cool;
- all six behavioural tests pass.
