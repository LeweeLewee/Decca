...
> must not be printed. Controlling document: specification **v1.7**.
...
adapter connects from the **sides**: fifteen green screw terminals per long
side, screws operated from above, conductors entering horizontally through
outward-facing ports. The two black vertical sockets carry the ESP32 and are
not wiring points.

**The move that decides everything.** v1.7 §5.3 requires the housing to clear
the **entire measured 9.00 mm block height across the whole 53.00 mm row on
both long sides**. So the base's long walls stop at the board top face, z 6.10,
leaving 17.90 mm of open side; the lid carries **no long-side skirt**, because
one would have to be dragged through the fitted conductors; and all retention,
fastening and lid support live at the two short ends.

**Retention is one ledge and two posts, and no fastener touches the board.**
The −X short edge sits under an integral fixed ledge 0.20 mm clear of the board
top. The +X end drops onto **two Ø2.60 locating posts through the measured
mounting holes** (60 mm across × 57 mm along, M3). v1.7 §4.6 replaced Rev C's
adjustable clamp with these, deleting a whole production part, two plinths, two
slotted M3 screws, two heat-set inserts and 3.60 mm of enclosure length.

**Screws through those holes are prohibited, and §4.6a records why.** The
terminal block is not the obstacle — a corner hole is outboard of it on both
axes, so an M3 cap head clears the block corner by 0.45 mm. The obstacle is the
**outermost conductor**, 3.767 mm from the hole centre: against the declared
2.60 mm ferrule that head fouls it by 0.28 mm. A Ø2.60 post clears it by
1.17 mm.

| File | Contents |
|---|---|
| `Decca_ESP32_Controller_Housing_fusion.py` | The parametric generator. Four entry points: `main` builds, `validate` runs the v1.7 §13 gates on the BRep solids, `export` writes the files below, `images` writes the renders. |
| `Decca_ESP32_Controller_Housing_verify.py` | The independent offline verifier. Reads **only** the exported STLs, against values typed in by hand. numpy is its only dependency. |
| `Decca_ESP32_Controller_Housing_slice.py` | The slicer-evidence harness. Drives the Bambu Studio CLI over the production set and reports real filament grams and print time. Exits with a plain explanation if Bambu Studio, Windows or the expected environment is absent. |
| `Decca_ESP32_Controller_Housing.f3d` | Editable archive. |
...
**Result:** 74.50 × 73.10 × 27.20 mm, **26.71 cm³ / 33.9 g** on solid volume,
...
PETG-HF at **31.73 g and 1 h 34 m 37 s**, zero support features.
...
failed**, 15 prototype gates open.

Rev A and Rev B production and coupon artefacts are removed, and both verifiers
fail if one reappears. `ESP32_Controller_PCB_Clamp_Adjustable.step` went the
same way under v1.7 §4.6.
