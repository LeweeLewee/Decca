...
The wall is **1.65 mm on the sides** — thick enough for its inner face to sit
...
> **The fit is the open finding.** The skirt is **36.20 × 17.20 mm** against a
> MEASURED opening of **35.20 × 15.30 mm** — 1.00 mm oversize across and
> **1.90 mm oversize up**. As modelled the part cannot enter the opening.
> Re-measure the real Perspex opening before printing a bezel. Build report
> §3.7 and §8.1.
...
Validation: **52/52 gates PASS** in Fusion and **46/46 PASS** in the independent
...
the four R3.40 corners — a stronger proof than point sampling, since any gap or
thin spot anywhere removes area.

| | mm |
|---|---:|
| Bezel face opening, at the front face | **32.900 × 15.600**, R1.750 |
| Inset-wall outer / inner envelope | **36.200 × 17.200** / 32.900 × 15.600 (flush, all four sides) |
| Wall / corner radii | **1.650 sides / 0.800 top+bottom** / R3.400 outer, R1.750 inner |
| Depth | 2.800 |
| Aperture | a straight bore — taper 0.00° |
| Fit | **0.500 interference** per horizontal side, **0.950 interference** per vertical side |
| **Effective clear optical opening** | **32.900 × 15.600** |

> **Three things the prototype must settle, and CAD cannot.**
>
> 1. **The fit.** It is not a question of force any more — on the measured
>    opening the part does not go in at all. Re-measure the opening. If it does
>    enter, the horizontal interference is resisted by the **1.65 mm side**
>    wall, about **70× stiffer in bending** than the original 0.40 mm, and
>    whether that flexes instead of stressing the Perspex is physical. Print
>    `Bezel_Fit_Gauge_revQ` first; it is mandatory, not advisable. Build report
>    §8.1 and §3.7.
> 2. **At least two continuous wall loops.** CAD proves the geometry admits
>    them — four on the sides, two top and bottom, never fewer than two through
>    the corners; only the production slicer preview can prove it lays them.
>    Build report §7.1 and §10 Stage 0b.
> 3. **The optical result.** The clear opening is 32.90 × 15.60 and the visible
>    lit band goes 8.100 → **8.450 mm** — more than Rev N, for the first time.
>    Whether the extra unlit board revealed below the active area looks right is
>    a powered-test decision, not a CAD result. Build report §5.

> **A forced modelling decision, since designed out.** While the face opening
> (15.35) was taller than the whole wall (15.20), a straight-walled aperture
> would have left the top and bottom wall runs detached from the bezel face, so
> the aperture tapered in Y at 36.10°. The fourth owner change made the face
> opening flush with the skirt on all four sides, which removed the condition:
> the aperture is now a straight bore, and the generator refuses to build if the
> two drift apart. Build report §3.3.

> **Four owner changes on top of the brief, each recorded and each reversible.**
> The two recessed adhesive pads are **deleted**; the inset-wall outer corner
> went **R2.00 → R3.00** because R2.00 did not match the real opening corner;
> the aperture was made **flush on the left and right**, with the loop rule
> clarified to **at least** two per side; and the **interference-fit
> refinement** added one outward 0.40 mm loop to each side face and moved the
> top and bottom walls 1.00 mm out each. Build report §3.5, §3.6 and §3.7.
