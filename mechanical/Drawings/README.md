...
print. It is NOT released.** The bezel is one connected manifold solid: the
Rev N face, envelope, external radii, 30.40 mm window width, R0.80 window
corners and both recessed adhesive pads are carried over unchanged, and the two
Rev N side rails are replaced by a single continuous 0.40 mm masking lip,
2.80 mm deep, around all four sides and all four corners of the opening.
...
| `Decca_OLED_Display_Bezel_revQ_front.png` | front view — the aperture reads as one clean opening |
| `Decca_OLED_Display_Bezel_revQ_rear.png` | rear view |
| `Decca_OLED_Display_Bezel_revQ_oblique.png` | oblique |
| `Decca_OLED_Display_Bezel_revQ_lip_oblique.png` | rear three-quarter — the lip as a continuous closed ring |
...
Validation: **30/30 gates PASS** in Fusion and **35/35 PASS** in the independent
offline mesh verifier. One shell, one lump, zero slivers; the lip is continuous
at every one of 1440 perimeter stations at three depths; wall 0.4000 mm min and
max; zero interference with the Perspex, the OLED glass or the frozen Rev P.5
carrier; and nothing at all behind the Perspex rear face.

> **Two things the prototype must settle, and CAD cannot.**
>
> 1. **The opening corner radius has never been measured** and is recorded
>    nowhere in this project. The Rev N side rails sat at y ±4.00 and never went
>    near a corner, so they prove nothing about it. `bezel_lip_corner_r` is
>    therefore a named **UNRESOLVED** parameter, set to the proven Rev N
>    rail-end relief of R0.60, which seats for any opening corner radius up to
>    **1.112 mm**. If the real corners are rounder than that the bezel will
>    stand proud — visibly, harmlessly, and fixably with one parameter. Print
>    `Bezel_Corner_Gauge_revQ` first to measure it. Build report §8.
> 2. **The lip costs 0.350 mm of lit screen height.** The clear opening goes
>    from 30.40 × 14.90 to **30.40 × 14.20 mm**, controlled now by the top and
>    bottom lip rather than by the bezel face, and the visible active band drops
>    from 8.100 to **7.750 mm** — all of it off the top. Whether that is
>    acceptable is a **powered test**, not a CAD result. Build report §5.
